...
def reg_station():
    ...
    if request.method=='POST':
        ...
        sql = "INSERT INTO ev_station(id,name,stype,num_charger,area,city,lat,lon,uname,pass,landmark,mobile,email,distance) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s)"
        
        val = (maxid,name,stype,num_charger,area,city,lat,lon,uname,pass1,landmark,mobile,email,'3')
...
def deletebyid():
    ...
    sid = request.args.get("sid")
    if sid:
        cursor.execute("DELETE FROM ev_station WHERE id = %s", (sid,))
        mydb.commit()
        msg = f"Record with ID {sid} was deleted."
